...
function useView<T>(view: string, limit = 100): FetchState<T[]> {
      ...
      useEffect(() => {
            supabase
                  ...
                  .select('*')
                  .limit(limit)
                  .then(({ data, error }: SupabaseReturnObjects) => {
                        setState({
                              data: (data as T[]) ?? [],
                              loading: false,
...
function useRpc<T>(
      ...
      useEffect(() => {
            supabase.rpc(fn, params).then(({ data, error }: SupabaseReturnObjects) => {
                  setState({
                        data: (data as T[]) ?? [],
                        loading: false,
                        error: error?.message ?? null,
...
export const useRevenueDaily = (limit = 300) => useView<RevenueDaily>('v_revenue_daily', limit);

export const useRevenueMonthly = () => useView<RevenueMonthly>('v_revenue_monthly');

export const useBestSelling = (n = 200) => useView<BestSellingItem>('v_best_selling_items', n);

export const useTopCustomers = (n = 200) => useView<TopCustomer>('v_top_customers', n);
...
export const useDailySnapshot = (date: string) => useRpc<DailySnapshot>('fn_daily_snapshot', { target_date: date });

export const useRevenueRange = (from: string, to: string) => useRpc<RevenueDaily>('fn_revenue_range', {
      from_date: from,
      to_date: to,
});

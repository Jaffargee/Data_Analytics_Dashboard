...
function SidebarContent({ onClose }: SidebarContentProps) {
      ...
      return (
            <div className="w-full flex-shrink-0 bg-bg-panel border-r border-bg-border flex flex-col h-full overflow-hidden">
                  ...
                  {/* Quick actions */}
                  <div className="px-3 py-3 border-b border-bg-border flex gap-2 flex-column">
                        {quickActions.map((a) => (
                              <Button
                                    key={a.path}
                                    size='xm'
                                    radius='full'
                                    variant="accent"
                                    icon={<Plus size={10} className="text-accent-gold" />}
                                    className="flex-1"
                                    onClick={onClose}
                              >
                                    <span className="font-mono text-accent-gold truncate">
                                          {a.label.replace('New ', '')}
                                    </span>
                              </Button>
                        ))}

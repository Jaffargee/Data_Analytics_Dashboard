...
function makeAssistantMessage(markdown: string): Omit<Message, 'id'> {
      return {
            ...
            content: renderTokens(
                  parseLLMContent(markdown, { enableLinks: true })
            ),
      };
}
...
export default function AIDataAnalyticsChat() {
      ...
      const handleSubmit = async (e?: React.FormEvent) => {
            ...
            try {
                  ...
                  const sqlRaw = await gpt_llm_query(
                        query,
                  ...
                  try {
                        ...
                        throw new Error(
                              'AI returned an unreadable response. Try rephrasing your question.'
                        );
                  }
                  ...
                  if (
                        ...
                        throw new Error(
                              queryResponse.error ??
                                    "I couldn't build a query for that question."
                        );
                  }
                  ...
                  const analysis = await gpt_llm_query(
                        payload,
                        ANALYSIS_INSTRUCTION
                  );
                  if (!analysis)
                        throw new Error('AI failed to analyse the results.');
      ...
      return (
            <div className="flex flex-col h-screen">
                  <TopBar
                        title="AI Analyst"
                        subtitle="Ask anything about your data in plain English"
                  />
...
function AssistantBubble({ content }: { content: React.ReactNode[] }) {
      return (
            <div className="flex justify-start pt-3">
                  ...
                  <div
                        className="w-full text-sm font-body text-ink-primary leading-7
                  ...
                  <div className="flex flex-col bg-bg-card border border-accent-gold/20 rounded-2xl px-3 py-2 focus-within:border-accent-gold/50 transition-all shadow-lg">
                        ...
                        <div className="flex items-center justify-between pt-1">
                              {isLoading ? (
                                    <Loader2
                                          size={17}
                                          className="text-accent-gold animate-spin ml-1"
                                    />
                              ) : (
                                    <button
                                          type="button"

...
export default function StockPage() {
      ...
      const critical = all.filter(
            (r) =>
                  ...
                  Number(r.stock_qty) <= Number(r.reorder_level) * 0.5
      ).length;
      const totalValue = all.reduce(
            (s, r) => s + Number(r.stock_qty) * Number(r.cost_price ?? 0),
            0
      );
                        ...
                        <Card>
                              ...
                              <div className="space-y-2">
                                    {filtered.map((r) => {
                                          ...
                                          const replenish = Number(
                                                r.replenish_level ?? reorder * 2
                                          );
                                          const pct =
                                                reorder > 0
                                                      ? Math.min(

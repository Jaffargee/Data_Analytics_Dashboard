...
export default function ProductsPage() {
      ...
      const maxRevenue = Math.max(
            ...allItems.map((i) => Number(i.total_revenue)),
            1
      );
      ...
      const filtered = useMemo(() => {
            ...
            rows = [...rows].sort((a, b) => {
                  const va = Number(
                        (a as unknown as Record<string, unknown>)[sortKey] ?? 0
                  );
                  const vb = Number(
                        (b as unknown as Record<string, unknown>)[sortKey] ?? 0
                  );
      ...
      const totalRevenue = allItems.reduce(
            (s, i) => s + Number(i.total_revenue),
            0
      );
      const totalQty = allItems.reduce(
            (s, i) => s + Number(i.total_qty_sold),
            0
      );
      const totalProfit = allItems.reduce(
            (s, i) => s + Number(i.gross_profit),
            0
      );
      const avgMargin = allItems.length
            ? allItems.reduce((s, i) => s + Number(i.margin_pct ?? 0), 0) /
              allItems.length
            : 0;

      const top10Chart = allItems.slice(0, 10).map((i) => ({
            label:
                  i.item_name.length > 14
                        ? i.item_name.slice(0, 14) + '…'
                        : i.item_name,
      ...
      return (
            <div className="flex-1 flex flex-col min-h-screen">
                  ...
                  <main className="flex-1 p-6 space-y-6">
                        <div className="grid grid-cols-1 md:gird-cols-2 xl:grid-cols-4 gap-4">
                              <StatCard
                                    label="Total Revenue"
                                    ...
                                    <table className="w-full text-sm">
                                          <thead>
                                                <tr className="border-b border-bg-border">
                                                      <th className="text-left pb-3 pr-4 text-xs font-body uppercase tracking-wider text-ink-muted">
                                                            #
                                                      </th>
                                                      <th className="text-left pb-3 pr-4 text-xs font-body uppercase tracking-wider text-ink-muted">
                                                            Item
                                                ...
                                                {filtered
                                                      ...
                                                      .map((r, i) => {
                                                            ...
                                                            return (
                                                                  ...
                                                                  >
                                                                        <td className="py-3 pr-4 text-ink-faint text-xs font-mono">
                                                                              {i +
                                                                                    1}
                                                                        </td>

...
async function generateSuggestions(): Promise<{ label: string; query: string }[]> {
      const stringified = JSON.stringify(EXAMPLE_QUERIES)
      const systemPrompt = `
            As a system message: Generate like 10 different random suggestion for a user to check for data to analyse.

            The response should be straight no text or explanation needed, and most importantly it should in json format
            Here is an example of the json format:
            ${stringified}
      `

      const resp = await llm_query('Generate 10 random suggestions for a user to check for data to analyse', systemPrompt)
      if(!resp) return []
      
      try {
            const clean = resp.replace(/```json|```/g, '').trim()
            return JSON.parse(clean) as { label: string; query: string }[]
      } catch {
            return []
      }
}
